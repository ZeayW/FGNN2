...
def load_data(options):
    ...
    with open(train_data_file, 'rb') as f:
        ...
        mask0=train_g.ndata['ntype']==0
        mask1=train_g.ndata['ntype']==1
        mask2=train_g.ndata['ntype']==2
        print(len(train_g.ndata['ntype'][mask0]),len(train_g.ndata['ntype'][mask1]),len(train_g.ndata['ntype'][mask2]))
...
if __name__ == "__main__":
    ...
    if options.test_iter:
        ...
        Loss = nn.CrossEntropyLoss()
        validate([valdataloader], 'adder_o', device, model,Loss, beta, options)
