...
def aig2dglG(nodes,edges):
    ...
    for n in nodes:
        nid = node2id[n[0]]
        if options.ntypes==1: n_inv[nid][0] = n[1]['inv']
    ...
    for eid, (src, dst, edict) in enumerate(edges):
        ...
        if options.ntypes==1: e_reverted[eid][0] = edict['is_reverted']
...
if __name__ == "__main__":
    ...
    for design, top_module in design_list:
        ...
        for vfile_pair in vfile_pairs:
            ...
            if options.ntypes==1: aig_nodes, aig_edges = putINV2edge(aig_nodes,aig_edges)
